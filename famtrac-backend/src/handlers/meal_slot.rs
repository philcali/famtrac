...
use crate::domain::{DependentId, FamilyId, MealSlot, MealSlotId, PermissionAction, RecipeId, Timestamp};
use uuid::Uuid;
...
pub async fn create_meal_slot<F: FamilyRepository, M: MealSlotRepository>(
    ...
    let recipe_id = request.recipe_id.and_then(|s| Uuid::parse_str(&s).ok().map(RecipeId));
